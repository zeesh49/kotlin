...
//import com.google.dart.compiler.backend.js.ast.JsNameOf;
//import com.google.dart.compiler.backend.js.ast.JsNumericEntry;
//import com.google.dart.compiler.backend.js.ast.JsSeedIdOf;
...
public class ClosureJsAstTranslator {
  ...
  private Node applySourceInfo(Node n, HasSourceInfo srcNode) {
    ...
    if (n != null && srcNode != null) {
      ...
      if (info != null && info.getSource() != null && info.getSource().getName() != null) {
        n.setStaticSourceFile(getClosureSourceFile(info.getSource().getName()));
        n.setLineno(info.getLine());
        n.setCharno(0);
      }
    }
  ...
  private Node transform(JsCase x) {
    ...
    for (Object element : x.getStatements()) {
  ...
  private Node transform(JsIf x) {
    Node n = IR.ifNode(transform(x.getIfExpression()), transformBody(x.getThenStatement(), x));
    if (x.getElseStatement() != null) {
      n.addChildToBack(transformBody(x.getElseStatement(), x));
    }
    return applySourceInfo(n, x);
  }
  ...
  private Node transform(JsNameRef x) {
    ...
    boolean isExternal = name == null/*z || !name.isObfuscatable()*/;
    if (x.getQualifier() != null) {
      n = IR.getprop(transform(x.getQualifier()), transformNameAsString(x.getIdent(), x));
      if (isExternal) {
        this.externalProperties.add(x.getIdent());
      }
    } else {
      n = transformName(x.getIdent(), x);
      if (isExternal) {
        this.externalVars.add(x.getIdent());
      } else if (name.getEnclosing() == program.getScope()) {
        this.globalVars.add(x.getIdent());
      }
    }
    applyOriginalName(n, x);
    return applySourceInfo(n, x);
  ...
  private Node transform(JsNumberLiteral x) {
    if (x instanceof JsNumberLiteral.JsIntLiteral){
        return IR.number(((JsNumberLiteral.JsIntLiteral) x).value);
    }
    else if (x instanceof JsNumberLiteral.JsDoubleLiteral){
        return IR.number(((JsNumberLiteral.JsDoubleLiteral) x).value);
    }
  ...
  private Node transform(JsObjectLiteral x) {
    ...
    for (Object element : x.getPropertyInitializers()) {
      ...
      if (propInit.getLabelExpr().getKind() == NodeKind.NUMBER) {
        ...
        key =
            transformNameAsString(((JsNameRef) propInit.getLabelExpr()).getIdent(), propInit
                .getLabelExpr());
      } else {
        key = transform(propInit.getLabelExpr());
      }
      Preconditions.checkState(key.isString(), key);
      n.addChildToBack(IR.propdef(IR.stringKey(key.getString()), transform(propInit.getValueExpr())));
    }
    return applySourceInfo(n, x);
  }
  ...
  private Node transform(JsThrow x) {
    Node n = IR.throwNode(transform(x.getExpression()));
    return applySourceInfo(n, x);
  }
  ...
  private Node transform(JsWhile x) {
    Node n =
        IR.forNode(IR.empty(), transform(x.getCondition()), IR.empty(), transformBody(x.getBody(), x));
    return applySourceInfo(n, x);
  }
  ...
  private Node transformLabel(JsName label) {
    ...
    return applySourceInfo(n, label.makeRef());
  }
  ...
  private Node transformName(JsName name) {
    ...
    return applySourceInfo(n, name.makeRef());
  }
  ...
  private Node transformNumberAsString(JsNumberLiteral literalNode) {
    Node irNode = Node.newString(literalNode.toString());
    return irNode;
  }
}
