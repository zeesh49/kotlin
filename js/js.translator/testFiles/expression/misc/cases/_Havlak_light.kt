...
class CFG {
    ...
    fun createNode(name: Int): BasicBlock {
        fun createAndRegisterNode(that: CFG, name: Int) : BasicBlock {
            val newNode = BasicBlock(name)
            that.basicBlockMap.put(name, newNode)
            return newNode
        }

        val node = basicBlockMap[name] ?: createAndRegisterNode(this, name)
...
class HavlakLoopFinder(val cfg: CFG, val lsg: LSG) {
    ...
    fun findLoops(): Int {
        ...
        while (w --> 0) {
            ...
            //
            // val workList = ArrayList<UnionFindNode>()
            val workList = LinkedList<UnionFindNode>()
            workList.addAll(nodePool)
            // val workList: jet.MutableList<UnionFindNode> = ArrayList_<UnionFindNode>(nodePool) //todo ?
...
//    println("15000 dummy loops");
    ...
    for (i in 1..15000) {
        var lsglocal = LSG()
        var finder = HavlakLoopFinder(cfg, lsglocal)
...
//    println("# of BBs  : " + numBasicBlocks)
    ...
    assertEquals(1522, lsg.getNumLoops())
    assertEquals(5053, numBasicBlocks)

//    lsg.calculateNestingLevel();

    println("Havlak: ${time} ms")
}

fun assertEquals(a: Int, b: Int) {
    if (a != b) {
        val msg = "expected: $a; actual: $b"
        println(msg)
