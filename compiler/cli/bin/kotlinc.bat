...
if not "%_KOTLIN_RUNNER%"=="" (
  "%_JAVACMD%" %_UTF8_AS_DEFAULT_ENCODING% %_JAVA_OPTS% "-Dkotlin.home=%_KOTLIN_HOME%" -cp "%_KOTLIN_HOME%\lib\kotlin-runner.jar" ^
    org.jetbrains.kotlin.runner.Main %*
) else (
  "%_JAVACMD%" %_UTF8_AS_DEFAULT_ENCODING% %_JAVA_OPTS% -noverify -cp "%_KOTLIN_HOME%\lib\kotlin-preloader.jar" ^
