...
// TODO: support annotations on types and types projections
// TODO: support type parameters on types and types projections
fun getKotlinTypeByKType(scriptDescriptor: ScriptDescriptor, kType: KType): KotlinType? {
    val classifier = kType.classifier
    if (classifier !is KClass<*>)
        throw UnsupportedOperationException("Type parameters are not supported as parameters in script template: $classifier")
...
private fun getTypeProjection(scriptDescriptor: ScriptDescriptor, kTypeProjection: KTypeProjection): TypeProjection {
    ...
    val classifier = kType.classifier
    if (classifier !is KClass<*>)
        throw UnsupportedOperationException("Type parameters are not supported as type projections in script template: $classifier")

    val type = getKotlinType(scriptDescriptor, classifier).let { if (kType.isMarkedNullable) TypeUtils.makeNullable(it) else it }
